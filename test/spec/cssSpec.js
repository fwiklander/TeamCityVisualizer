...
var testResponse = {canceledInfo: {user: {name: 'Unit Test'}, text: 'This is a unit test'}};
...
describe("CSS changes", function(){
  beforeEach(function(){
    setFixtures(sandbox({id: 'blinking', class: 'buildOngoing'}));
    appendSetFixtures(sandbox({id: 'nonBlinking'}));
  ...
  describe("Canceled build", function(){
    it("should call the build endpoint with the build number", function(){
      ...
      request.respondWith({status: 200, responseText: JSON.stringify(testResponse)});
